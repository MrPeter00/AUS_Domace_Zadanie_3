...
namespace structures
{
	...
	inline BinarySearchTree<K, T>& BinarySearchTree<K, T>::operator=(const BinarySearchTree<K, T>& other)
	{
		if (this != &other)
		{
			...
			Tree<TableItem<K, T>*>::PreOrderTreeIterator* iterCurrent = new Tree<TableItem<K, T>*>::PreOrderTreeIterator(other.binaryTree_->getRoot());
			Tree<TableItem<K, T>*>::PreOrderTreeIterator* iterEnd = new Tree<TableItem<K, T>*>::PreOrderTreeIterator(nullptr);

			while (*iterCurrent != *iterEnd)
			{
				TableItem<K, T>* item = **iterCurrent;
				insert(item->getKey(), item->accessData());
				++(*iterCurrent);	//preto prefix lebo len ten operator je definovany
			}
	...
	inline T& BinarySearchTree<K, T>::operator[](const K key)
	{
		...
		if (found)
			...
			throw std::logic_error("BST does not contain this key!");
	}
	...
	inline void BinarySearchTree<K, T>::insert(const K& key, const T& data)
	{
		...
		BinaryTreeNode< TableItem<K, T>*>* binaryTreeNode = new BinaryTreeNode< TableItem<K, T>*>(item);
	/*	if (tryToInsertNode(binaryTreeNode))	//ak sa mi nepodari
		{
			size_++;
		}
		else
		{
			...
			throw std::logic_error("Table already contains an item with this key!");
		}*/
		if (!tryToInsertNode(binaryTreeNode))
		{
			delete item;
			delete binaryTreeNode;
	...
	{
		bool found = false;
		BinaryTreeNode<TableItem<K, T>*>* binaryTreeNode = findBSTNode(key, found);
		if (found)
		{
			extractNode(binaryTreeNode);
			T result = binaryTreeNode->accessData()->accessData();
			delete binaryTreeNode->accessData();
			delete binaryTreeNode;
			size_--;
			return result;
		}
		...
		{
			throw std::logic_error("Table does not contain an item with this key!");
		}
	}
	...
	{
		bool found = false;
		BSTTreeNode* item = findBSTNode(key, found);	//mozem aj takto bez <...> lebo je tam typename dakde :D
		return found;
	}
