...
namespace structures
{
	...
	inline SequenceTable<K, T>& SequenceTable<K, T>::operator=(const SequenceTable<K, T>& other)
	{
		...
		{
			clear();
			// *list_ = *other.list_; // v liste su adresy, cize toto by nebolo deep copy ale iba shallow copy
			// toto je spravne!
	...
	inline T& SequenceTable<K, T>::operator[](const K key)
	{
		...
		if (item == nullptr)
			throw std::logic_error("No data found with this key");
	...
	{
		if (containsKey(key))
			throw std::logic_error("Table already contains data with this key!");
	...
	{
		//zmazem pomocne itemy
		for (TableItem<K, T>* item : *list_)
		{
			delete item;
		}
		//zmazem adresy v zozname
		list_->clear();
	}
