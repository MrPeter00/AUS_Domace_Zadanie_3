...
namespace structures
{
	...
	{
		TreeNode<T>* result = shallowCopy();	//najprv si spravim plytku kopiu
		for (int i = 0; i < degree(); i++)
		{
			TreeNode<T>* son = getSon(i); // vsetkych synov nahradim ich hlbokou kopiou
			if (son != nullptr)
			{
				result->replaceSon(son->deepCopy(), i);	//rekurzivne
			}
		}
		return result;
	}
	...
	{
		if (isRoot())
			throw std::logic_error("Node is a root. It has no brothers!");

		// uzol je sam sebe prvym bratom
	...
	{
		size_t result = 1; //zacinam pocitat od seba
		for (int i = 0; i < degree(); i++)
		{
	...
	{
		// staci to v tom pripade ak mam spravne implementovany destruktor 
		// v konkretnej implementacii TreeNode-u
	...
	{
		TreeNode<T>* current = path_->front();	//ekvivalent pre peek()
