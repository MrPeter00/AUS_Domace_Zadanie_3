...
namespace structures
{
	...
	inline ArrayList<T>& ArrayList<T>::operator=(const ArrayList<T>& other)
	{
		if (this != &other)
		{
			...
			size_ = other.size_;
			//array_ = other.size_; tu priradujeme smerniky to je zle
			//*array_ = *(other.array_); nemozeme pouzit toto pretoze maju ine velkosti
			delete array_;
			array_ = new Array<T>(*other.array_);
		}
		return *this;
	...
	inline T ArrayList<T>::removeAt(const int index)
	{
		...
		Array<T>::copy(*array_, static_cast<int>(index + 1), 
			*array_, static_cast<int>(index),
			static_cast<int>(size_ - index - 1)); //odkial kopirujem, z ktoreho prvu kam, do akeho prvku a ko¾ko toho kopirujem
